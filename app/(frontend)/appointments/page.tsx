        ...
        <div className="min-h-screen bg-gray-50 flex flex-col h-screen overflow-hidden">
            <div className="bg-white border-b border-gray-200 px-4 md:px-6 py-4 flex flex-col md:flex-row items-center justify-between shrink-0 gap-4">
                <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                    <h1 className="text-xl md:text-2xl font-bold text-gray-900 self-start md:self-auto">Appointments</h1>
                    <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200 w-full md:w-auto justify-center">
                        <button
                            onClick={() => setView('list')}
                            className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-xs font-bold transition-all flex justify-center items-center gap-2 ${view === 'list'
                        ...
                        <button
                            ...
                            className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-xs font-bold transition-all flex justify-center items-center gap-2 ${view === 'calendar'
                                ? "bg-white text-blue-900 shadow-sm"
                                : "text-gray-500 hover:text-gray-700"
                                }`}
                        >
                                        ...
                                        {loading && appointments.length === 0 ? (
                                            ...
                                            appointments.map((apt) => (
                                                <tr key={apt._id} className="hover:bg-gray-50/50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <span className="text-sm font-bold text-gray-900">{settings.symbol}{apt.totalAmount.toLocaleString('id-ID', { maximumFractionDigits: 0 })}</span>
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <div key={apt._id} className="p-4 hover:bg-gray-50/50 transition-colors flex flex-col gap-3">
                                                <div className="flex justify-between items-start">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <h3 className="text-sm font-bold text-gray-900">{format(new Date(apt.date), "dd MMM yyyy")}</h3>
                                                            <p className="text-xs text-blue-700 font-semibold uppercase">{apt.startTime} - {apt.endTime}</p>
                                                        </div>
                                                    </div>
                                                    <div className="relative dropdown-trigger">
                                                        <button onClick={() => setActiveDropdown(activeDropdown === apt._id ? null : apt._id)} className="p-2 -mr-2 text-gray-400 hover:text-blue-900 rounded-lg">
                                                            <MoreVertical className="w-5 h-5" />
                                                        </button>
                                                        {activeDropdown === apt._id && (
                                                            <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-xl z-50 py-1">
                                                                ...
                                                                <div className="h-px bg-gray-100 my-1" />
                                                                <button onClick={() => { handleDelete(apt._id); setActiveDropdown(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition-colors">
                                                ...
                                                <div className="grid grid-cols-2 gap-2 text-sm pt-1">
                                                    <div>
                                                        ...
                                                        {apt.customer.phone && <p className="text-xs text-gray-500">{apt.customer.phone}</p>}
                                                    </div>
                                                    <div>
                                                        <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">Staff</p>
                                                        <p className="font-medium flex items-center gap-1.5"><User className="w-3.5 h-3.5 text-blue-400" />{apt.staff.name}</p>
                                                    </div>
                                                </div>

                                                <div>
                                                    <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">Services</p>
                                                ...
                                                <div className="flex justify-between items-end pt-3 border-t border-gray-100 mt-2">
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <span className="text-lg font-black text-gray-900">{settings.symbol}{apt.totalAmount.toLocaleString('id-ID', { maximumFractionDigits: 0 })}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal} title={editingAppointment ? "Edit Appointment" : "New Appointment"}>
                <form onSubmit={handleSubmit}>
                            ...
                            <div className="text-right space-y-1">
                                ...
                                <div className="flex items-center justify-end gap-2">
                                    ...
                                    <span className="text-2xl font-black">
                                        {settings.symbol}{((services.filter(s => formData.serviceIds.includes(s._id)).reduce((a, b) => a + b.price, 0) * (1 + settings.taxRate / 100)) - (formData.discount || 0)).toLocaleString('id-ID', { maximumFractionDigits: 0 })}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 mt-6">
                        <button type="button" onClick={closeModal} className="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 font-medium" disabled={isSubmitting}>Cancel</button>
                        <FormButton type="submit" loading={isSubmitting} className="w-full sm:w-auto">
