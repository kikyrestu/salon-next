                                    ...
                                    {loading && products.length === 0 ? (
                                        ...
                                        products.map((product) => (
                                            <tr key={product._id} className="hover:bg-gray-50/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span className="text-sm font-bold text-gray-900">{settings.symbol}{product.price.toLocaleString('id-ID', { maximumFractionDigits: 0 })}</span>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div key={product._id} className="p-4 hover:bg-gray-50/50 transition-colors flex flex-col gap-3">
                                            <div className="flex justify-between items-start">
                                                ...
                                                <div className="relative dropdown-trigger">
                                                    <button onClick={() => setActiveDropdown(activeDropdown === product._id ? null : product._id)} className="p-2 -mr-2 text-gray-400 hover:text-blue-900 rounded-lg">
                                                        <MoreVertical className="w-5 h-5" />
                                                    </button>
                                                    {activeDropdown === product._id && (
                                                        <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-xl z-50 py-1 font-medium">
                                                            <PermissionGate resource="products" action="edit">
                                                                <button onClick={() => { openModal(product); setActiveDropdown(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50">
                                                                    <Edit className="w-4 h-4 text-blue-600" /> Edit Details
                                                                </button>
                                                            </PermissionGate>
                                                            <div className="h-px bg-gray-100 my-1" />
                                                            <PermissionGate resource="products" action="delete">
                                                                <button onClick={() => { handleDelete(product._id); setActiveDropdown(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50">
                                            ...
                                            <div className="grid grid-cols-2 gap-2 text-sm pt-1 border-t border-gray-50 mt-1">
                                                <div>
                                                    <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">Stock</p>
                                                    <div className="flex items-center gap-1.5 font-semibold">
                                                        <span className={product.stock <= 5 ? "text-red-600" : "text-gray-900"}>{product.stock} units</span>
                                                        {product.stock <= 5 && <AlertCircle className="w-3.5 h-3.5 text-red-500" />}
                                                    </div>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">Brand</p>
                                                    <p className="font-semibold text-gray-900 capitalize">{product.brand || 'No Brand'}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex justify-between items-end pt-2 mt-1">
                                                ...
                                                <div className="text-right">
                                                    ...
                                                    <span className="text-lg font-black text-gray-900">{settings.symbol}{product.price.toLocaleString('id-ID', { maximumFractionDigits: 0 })}</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal} title={editingProduct ? "Edit Product" : "Add Product"}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 mt-6 pt-2">
                        <button type="button" onClick={closeModal} className="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 font-medium transition-colors text-center">Cancel</button>
                        <FormButton type="submit" loading={submitting} className="w-full sm:w-auto">
