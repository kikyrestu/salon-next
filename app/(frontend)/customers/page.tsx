...
export default function CustomersPage() {
    ...
    const [pagination, setPagination] = useState<PaginationState>({ total: 0, page: 1, limit: 10, pages: 0 });
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 sm:p-8">
                    ...
                    <div className="overflow-x-auto">
                                ...
                                {loading && customers.length === 0 ? (
                                    ...
                                    customers.map((customer) => (
                                        <tr key={customer._id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className="text-sm font-bold text-gray-900">{settings.symbol}{customer.totalPurchases.toLocaleString('id-ID', { maximumFractionDigits: 0 })}</span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                {customer.packageSummary?.hasPackage ? (
                                                    <div className="space-y-0.5">
                                                        <div className="text-xs font-bold text-amber-700">
                                                            Aktif: {customer.packageSummary.activePackages} paket
                                                        </div>
                                                        <div className="text-[11px] text-gray-600">
                                                            Sisa kuota: <span className="font-bold text-gray-900">{customer.packageSummary.totalRemainingQuota}</span>
                                                        </div>
                                                    </div>
                                                ) : (
                        ...
                        <div className="md:hidden flex flex-col divide-y divide-gray-100">
                            {loading && customers.length === 0 ? (
                                ...
                                customers.map((customer) => (
                                    <div key={customer._id} className="p-4 flex flex-col gap-3 relative hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex items-center gap-2 mt-0.5">
                                                        ...
                                                        <span className={`inline-flex items-center px-2 rounded-[4px] text-[9px] font-bold uppercase tracking-wide border ${customer.status === "active"
                                                            ? "bg-green-50 text-green-700 border-green-200"
                                                            : "bg-gray-50 text-gray-600 border-gray-200"
                                                            }`}>
                                                            {customer.status}
                                                        </span>
                                                        <span className="text-[10px] text-gray-400 font-medium">Joined {new Date(customer.createdAt).toLocaleDateString()}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="absolute top-4 right-2 dropdown-trigger">
                                            ...
                                            {activeDropdown === customer._id && (
                                                <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-xl z-50 py-1 overflow-hidden font-medium">
                                                    <PermissionGate resource="customers" action="edit">
                                                        <button onClick={() => { void openHistoryModal(customer); setActiveDropdown(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-amber-50 transition-colors">
                                                            <History className="w-4 h-4 text-amber-600" /> View History
                                                        </button>
                                                    </PermissionGate>
                                                    <PermissionGate resource="customers" action="edit">
                                                        <button onClick={() => { openModal(customer); setActiveDropdown(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50 transition-colors">
                                                            <Edit className="w-4 h-4 text-blue-600" /> Edit Details
                                                        </button>
                                                    </PermissionGate>
                                                    <div className="h-px bg-gray-100 my-1" />
                                                    <PermissionGate resource="customers" action="delete">
                                                        <button onClick={() => { handleDelete(customer._id); setActiveDropdown(null); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition-colors">
                                        ...
                                        <div className="space-y-1 bg-gray-50/50 p-3 rounded-lg border border-gray-100 mt-1">
                                            ...
                                            {customer.email ? (
                                                ...
                                                <span className="text-xs text-gray-400 italic flex items-center gap-2"><Mail className="w-3.5 h-3.5 text-gray-400 shrink-0" />No email provided</span>
                                            )}
                                            {customer.address && (
                                        ...
                                        <div className="flex justify-between items-center text-sm pt-1 mt-1 border-t border-gray-50">
                                            <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Total Purchases</p>
                                            <span className="font-black text-blue-900">{settings.symbol}{customer.totalPurchases.toLocaleString('id-ID', { maximumFractionDigits: 0 })}</span>
                                        </div>
                                        {customer.packageSummary?.hasPackage && (
                                            <div className="flex justify-between items-center text-sm pt-1 mt-1 border-t border-gray-50">
                                                <p className="text-[10px] text-amber-600 uppercase font-bold tracking-wider">Sisa Kuota Paket</p>
                                                <span className="font-black text-amber-700">{customer.packageSummary.totalRemainingQuota}</span>
                                            </div>
                                        )}
                                    </div>
                                ))
            ...
            <Modal
                ...
                title={historyCustomer ? `History Customer - ${historyCustomer.name}` : "History Customer"}
            >
                    ...
                    {historyLoading ? (
                        ...
                        <>
                            <section className="space-y-2">
                                <h3 className="text-sm font-bold text-gray-900">Transaksi Invoice Terbaru</h3>
                                {historyData.invoices.length === 0 ? (
                                    <p className="text-xs text-gray-500">Belum ada transaksi invoice.</p>
                                ) : (
                                    <div className="space-y-1">
                                        {historyData.invoices.map((inv) => (
                                            <div key={inv._id} className="text-xs border border-gray-200 rounded-lg p-2 bg-gray-50">
                                                ...
                                                <div className="mt-1 text-gray-600">
                                                    {settings.symbol}{Number(inv.totalAmount || 0).toLocaleString('id-ID', { maximumFractionDigits: 0 })} • {inv.status} • {inv.paymentMethod}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <section className="space-y-2">
                                <h3 className="text-sm font-bold text-gray-900">Riwayat Beli Paket</h3>
                                {historyData.packageOrders.length === 0 ? (
                                    <p className="text-xs text-gray-500">Belum ada pembelian paket.</p>
                                ) : (
                                    <div className="space-y-1">
                                        {historyData.packageOrders.map((order) => (
                                            <div key={order._id} className="text-xs border border-amber-200 rounded-lg p-2 bg-amber-50">
                                                <div className="flex items-center justify-between gap-2">
                                                    <span className="font-bold text-amber-900">{order.packageSnapshot?.name || 'Paket'}</span>
                                                    <span className="text-amber-700">{order.status}</span>
                                                </div>
                                                <div className="mt-1 text-amber-800">
                                                    {settings.symbol}{Number(order.totalAmount || 0).toLocaleString('id-ID', { maximumFractionDigits: 0 })}
                            ...
                            <section className="space-y-2">
                                <h3 className="text-sm font-bold text-gray-900">Riwayat Pemakaian Paket</h3>
                                {historyData.packageUsage.length === 0 ? (
                                    <p className="text-xs text-gray-500">Belum ada pemakaian paket.</p>
                                ) : (
                                    <div className="space-y-1">
                                        {historyData.packageUsage.map((usage) => (
                                            <div key={usage._id} className="text-xs border border-blue-200 rounded-lg p-2 bg-blue-50">
                                                <div className="flex items-center justify-between gap-2">
                                                    <span className="font-bold text-blue-900">{usage.serviceName}</span>
                                                    <span className="text-blue-700">-{usage.quantity} kuota</span>
                                                </div>
