...
export default function ServicesPage() {
    ...
    const [includeProductInCommission, setIncludeProductInCommission] = useState(false);
    ...
    const handleCategorySubmit = async (e: React.FormEvent) => {
        ...
        try {
            const url = editingCategory ? `/api/service-categories/${editingCategory._id}` : "/api/service-categories";
            const res = await fetch(url, {
    ...
    const handleCategoryDelete = async (id: string) => {
        if (!confirm("Are you sure you want to delete this category? Services belonging to it may no longer be listed under it.")) return;
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-8">
                    ...
                    <div className="overflow-x-auto overflow-y-visible">
                                ...
                                {loading && services.length === 0 ? (
                                    ...
                                    services.map((service) => (
                                        <tr key={service._id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className="text-sm font-bold text-gray-900">{settings.symbol}{service.price.toLocaleString('id-ID', { maximumFractionDigits: 0 })}</span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="md:hidden flex flex-col divide-y divide-gray-100 border-t border-gray-100">
                            {loading && services.length === 0 ? (
                                ...
                                services.map((service) => (
                                    <div key={service._id} className="p-4 hover:bg-gray-50/50 transition-colors flex flex-col gap-3">
                                        <div className="flex items-center gap-3">
                                            <div className="p-2 bg-blue-50 rounded-lg">
                                                <Scissors className="w-4 h-4 text-blue-900" />
                                            </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2 text-sm pt-1 border-t border-gray-50 mt-1">
                                            <div>
                                                ...
                                                <p className="font-semibold text-gray-900 flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-blue-400" /> {service.duration} min</p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">Gender</p>
                                                <p className="font-semibold text-gray-900 capitalize">{service.gender}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex justify-between items-end pt-2 mt-1">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <span className="text-lg font-black text-gray-900">{settings.symbol}{service.price.toLocaleString('id-ID', { maximumFractionDigits: 0 })}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))
                    ...
                    <div className="overflow-x-auto">
                                ...
                                {categories.length === 0 ? (
                                    ...
                                    categories.map((cat) => (
                                        <tr key={cat._id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`text-[10px] uppercase tracking-widest font-black px-2.5 py-1 rounded-full border ${cat.status === 'active' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-100 text-gray-700 border-gray-200'}`}>
                                                    {cat.status}
                                                </span>
                                            </td>
                        ...
                        <div className="md:hidden flex flex-col divide-y divide-gray-100 border-t border-gray-100">
                            {categories.length === 0 ? (
                                ...
                                categories.map((cat) => (
                                    <div key={cat._id} className="p-4 hover:bg-gray-50/50 transition-colors flex flex-col gap-3">
                                        ...
                                        <div className="flex justify-end gap-2 pt-3 border-t border-gray-50 mt-1">
                                            <PermissionGate resource="services" action="edit">
                                                <button onClick={() => openCategoryModal(cat)} className="px-4 py-2 border border-gray-200 text-gray-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors text-xs font-bold flex items-center gap-2 w-full sm:w-auto justify-center">
                                                    <Edit className="w-3.5 h-3.5" /> Edit
                                                </button>
                                            </PermissionGate>
                                            <PermissionGate resource="services" action="delete">
                                                <button onClick={() => handleCategoryDelete(cat._id)} className="px-4 py-2 border border-gray-200 text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors text-xs font-bold flex items-center gap-2 w-full sm:w-auto justify-center">
            ...
            {/* Service Modal */}
            <Modal isOpen={isServiceModalOpen} onClose={closeServiceModal} title={editingService ? "Edit Service" : "Add New Service"} size="xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <FormInput
                                ...
                                onChange={(e) => setServiceFormData({ ...serviceFormData, commissionValue: parseFloat(e.target.value) || 0 })}
                                min="0"
                            />
                            <p className="text-xs text-gray-500 -mt-2">
                                Nilai ini adalah rupiah tetap per layanan, bukan persentase dari harga layanan.
                            </p>
                        </div>
                        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 space-y-3">
                            <div className="flex items-center justify-between gap-3">
                                <span className="text-sm font-semibold text-gray-700">Opsi Komisi POS</span>
                                <span className="text-xs font-bold px-2 py-1 rounded-full bg-blue-100 text-blue-700">Ringkas</span>
                            </div>
                            ...
                            <label className="flex items-center gap-2 text-sm text-gray-700">
                                <input
                                    ...
                                    onChange={(e) => setIncludeProductInCommission(e.target.checked)}
                                />
                                Komisi juga untuk produk
                            </label>

                            <p className="text-xs text-gray-500">
                                Saat ini perhitungan komisi utama tetap dari nominal layanan. Produk belum dihitung otomatis sebagai komisi.
                            </p>
                        </div>
                    </div>
                            ...
                            <div className="space-y-3 mt-4">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <SearchableSelect
                                        ...
                                        onChange={(val) => setServiceFormData({
                                            ...serviceFormData,
                                            waFollowUp: {
                                                ...serviceFormData.waFollowUp,
                                                firstTemplateId: val,
